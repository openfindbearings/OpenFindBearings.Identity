...
| v2.5.0 | 2026-07-07 | 新增 SMS 短信验证码端点 /api/sms/send-code；AuthorizationController 新增 grant_type=sms 处理流程；maui-client 允许授权类型新增 sms；新增 SmsCodeTypeConstants.Register 常量 |
| v2.6.0 | 2026-08-20 | Scope 资源模型扩展：新增 ApiResourceConstants.SyncApi（openfindbearings-sync）；api:admin 关联双资源 [openfindbearings-api, openfindbearings-sync]，Admin token 可访问全部后台微服务；api:sync/api:maui/api:web 保持 openfindbearings-api |
| v2.7.0 | 2026-08-20 | 用户管理支持彻底删除：新增 DELETE /api/account/admin/users/{id}/permanent（物理删除，仅限已软删除用户，禁止删除当前管理员本人）；用户列表支持 includeDeleted=true 查询参数显示已软删除用户（GetPagedAsync 增加 includeDeleted 参数，UserQueryRequest 新增 IncludeDeleted 字段） |
| v2.8.0 | 2026-08-25 | 修复 RP-Initiated Logout ID2052 错误：admin_client 和 web-client 补充 `ept:end_session` 端点权限（OpenIddict 校验 post_logout_redirect_uri 时要求匹配客户端拥有 EndSession 权限，缺失导致校验失败） |
...
| Mobile App | 公开注册（SignUp + realm）+ 密码流程登录 / 短信验证码流程登录 | 自行传入 realm |
...
| Scope | 名称 | 关联资源（token 受众 aud） | 所属租户 |
|-------|------|------|----------|
...
| GET | `/api/account/admin/users` | 分页获取用户列表（管理员，支持 `includeDeleted=true` 显示已软删除用户） | OpenIddict Bearer + SuperAdmin/Admin |
...
| DELETE | `/api/account/admin/users/{id}/permanent` | 彻底删除用户（物理删除，仅限已软删除用户，禁止删除当前管理员本人） | OpenIddict Bearer + SuperAdmin/Admin |
...
### SMS 验证码端点（SmsController）

| 方法 | 路由 | 用途 | 认证 |
|------|------|------|------|
| POST | `/api/sms/send-code` | 发送短信验证码（含 `SmsCodeType` 参数：Register 等） | 匿名 |

AuthorizationController 新增 `grant_type=sms` 处理流程：接收手机号 + 短信验证码，验证通过后直接颁发身份令牌。maui-client 的允许授权类型新增 `sms`，配合 MAUI 客户端完成短信验证码登录流程。
...
│   ├── AuthorizationController.cs  # OAuth 端点 (authorize/token/userinfo/logout) + grant_type=sms
│   ├── HomeController.cs           # MVC: 仪表盘
│   ├── LoginController.cs          # MVC: 登录
│   ├── ProfileController.cs        # MVC: 更改密码
│   ├── ScopeController.cs          # MVC: Scope 管理
│   ├── SmsController.cs            # API: 短信验证码发送
...
│   └── SmsCodeTypeConstants.cs     # 短信验证码类型常量（Register 等）
├── Views/                          # MVC 视图
│   ├── Home/
│   ├── Users/
│   ├── Application/
