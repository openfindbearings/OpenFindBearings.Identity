...
**版本：** v2.13.0
**日期：** 2026-09-11
**状态：** 文档与代码同步（v2.12.0 的断链引用、管理台 UI 功能清单、中间件描述等按代码实际情况修正）
...
- **UI 组件（`_AudienceEditor.cshtml` 共享 partial）**：Keycloak 式行级编辑器，Scope 的 Create 和 Edit 页面复用：
  ...
  - Scope 列表页新增受众列（`Resources` 列表，逗号分隔），无受众显示"—"
...
> `qq`/`biometric` 仅存在于 `Constants/GrantTypeConstants.cs` 与 `AuthorizationController` 的 `NotImplementedException` handler，未在 `ServiceExtensions` 注册，当前不可达。
...
- 详见《OpenFindBearings.Taro/doc/01-架构设计/移动端登录与商户入驻设计-v1.1.0.md》。
...
UseMiddleware<TenantContextMiddleware>()  ← 解析 realm
UseAuthentication()
UseAuthorization()
UseMiddleware<AuditLogMiddleware>()       ← 审计日志
UseMiddleware<SecurityHeadersMiddleware>() ← X-Frame-Options: DENY, X-Content-Type-Options: nosniff
```

**安全头中间件**：`SecurityHeadersMiddleware` 在响应头注入 `X-Frame-Options: DENY`（防点击劫持）和 `X-Content-Type-Options: nosniff`（防 MIME 嗅探），对所有请求生效。
...
| 功能 | 说明 |
|---|---|
| 列表 | 分页表格，显示名称/显示名/描述/**受众列**（逗号分隔，无受众显示"—"） |
| 搜索 | 文本搜索 |
...
独立页面（渐变登录背景风格），表单：当前密码/新密码/确认新密码（最少 6 字符），支持 return URL 回跳。
...
2. **API 侧两个 bug**（API 项目，非 Identity）：① 商户审核门缺失；② `PermissionService.IsMerchantAdmin()` 读 JWT claim 恒 false。详见《移动端登录与商户入驻设计-v1.1.0》与 API 文档。
...
- 移动端登录/注册/商户入驻的**前端与 BFF 侧设计**见《OpenFindBearings.Taro/doc/01-架构设计/移动端登录与商户入驻设计-v1.1.0.md》；本文是 **Identity 侧权威认证设计**，两者交叉引用、不重复。
- 商户成员模型/状态机/apply 契约的**业务侧**见 API《06-商户入驻与认证体系》（需与本文 §7.4/§10.3 及 Taro v1.1.0 对齐升版）。
