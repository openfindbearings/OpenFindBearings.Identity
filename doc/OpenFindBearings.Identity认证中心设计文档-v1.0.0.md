...
| POST | `/api/account/admin/users/{id}/restore` | 恢复已软删除的用户 | **待实现** |
...
需要在 `SeedData.cs` 中添加 `admin_client` 的注册代码，配置为机密客户端，使用 authorization_code + PKCE 流程：

```csharp
// 待添加到 SeedData.cs 的 SeedClientsAsync 方法中
