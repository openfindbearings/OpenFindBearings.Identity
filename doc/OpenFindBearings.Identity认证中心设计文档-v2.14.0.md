...
**版本：** v2.14.0
**日期：** 2026-09-12
**状态：** 文档与代码同步（v2.14.0 校对商户入驻相关交叉引用与已修复标记）
...
| S9 | 服务间用户查询 | 未动 | API 依赖的 `/api/users/by-phone` 等仍不存在（员工邀请链路待落地）；方案不变：API 持 M2M token 调 `/api/account/admin/users` 或 Identity 补服务间端点。 | P2/P4 |
